...
export async function aoc(_year: number, _day: number, _part: 1 | 2) {
  ...
  if (Date.now() > puzzleDropTime) puzzleDrop = Promise.resolve()
  else puzzleDrop = new Promise(r => setTimeout(r, puzzleDropTime - Date.now()))
...
export function test(input: string, output1?: number, output2?: number) {
  if (input.trim()) tests.push([input.trim(), output1, output2])
}
