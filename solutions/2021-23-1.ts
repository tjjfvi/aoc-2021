...
solution(async input => {
  ...
  let cols = arr(4).map(x => [rows[0][x], rows[1][x]])
  ...
  function pathfind(
    ...
    if (cols.every((x, i) => [...x].filter(x => x === i).length === 2)) {
      return eng
    }
    ...
    for (let i = 0; i < hallway.length; i++) {
      ...
      hallway[i] = undefined
      if (blocked(i, [2, 4, 6, 8][val])) continue
      hallway[i] = val
      let newCols = cols.map(x => x.slice())
      let newHallway = hallway.slice()
      newHallway[i] = undefined
      let colInd = cols[val].findIndex(x => x === undefined)
